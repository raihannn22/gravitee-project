...
import my.gravitee.extension.policy.FooHeaderCheckPolicy;
import my.gravitee.extension.policy.FooHeaderCheckPolicyConfiguration;
...
@GatewayTest
@DeployApi("/my/gravitee/extension/policy/apis/FooHeaderCheck.json")
public class FooHeaderCheckPolicyIntegrationTest extends AbstractPolicyTest<FooHeaderCheckPolicy, FooHeaderCheckPolicyConfiguration> {
    ...
    public void shouldTestFooHeaderCheckPolicyDeployedOnAnInMemoryGateway(WebClient client) throws Exception {
        wiremock.stubFor(post("/backend").willReturn(ok("response from backend")));

        io.vertx.core.json.JsonObject jsonBody = new io.vertx.core.json.JsonObject();
        jsonBody.put("greeting", "Hello From Gravitee!");

        final TestObserver<HttpResponse<Buffer>> obs = client.post("/test")
            .rxSendJsonObject(jsonBody)
            .test();
        ...
        var requests = wiremock.findAll(postRequestedFor(urlPathEqualTo("/backend")));
        assertThat(requests).hasSize(1);
        byte[] receivedBody = requests.get(0).getBody();
        HelloRequest requestMessage = HelloRequest.parseFrom(receivedBody);
        assertThat(requestMessage.getGreeting()).isEqualTo("Hello From Gravitee!");
    }
}